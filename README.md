...
The worker also passes `--remote-components ejs:github` so yt-dlp can
download YouTube's JS challenge solver components when needed (without this,
yt-dlp exits `rc=0` but produces no subtitle file on many videos).

If yt-dlp returns `rc=0` but no subtitle file is found, check the worker
logs — it prints both the full temp-dir listing and the matched subtitle
files. The worker accepts `.srt`, `.vtt`, and `.json3` (YouTube's timed-text
JSON) and parses each natively, so a missing `.srt` after `--convert-subs`
is no longer fatal as long as `.vtt` or `.json3` was written.

...
SUB_EXTS = (".srt", ".vtt", ".json3", ".srv3", ".srv2", ".srv1", ".ttml")


def _pick_subtitle_file(workdir: Path) -> Path | None:
    all_files = sorted(workdir.glob("*"))
    print(f"[yt-dlp] temp dir files after run: {[p.name for p in all_files]}", flush=True)
    subs = [p for p in all_files if p.suffix.lower() in SUB_EXTS]
    print(f"[yt-dlp] found subtitle files: {[p.name for p in subs]}", flush=True)
    if not subs:
        return None
    # prefer english, then preferred extension order
    def score(p: Path) -> tuple[int, int]:
        name = p.name.lower()
        lang_score = 0 if ".en" in name else 1
        try:
            ext_score = SUB_EXTS.index(p.suffix.lower())
        except ValueError:
            ext_score = 99
        return (lang_score, ext_score)
    subs.sort(key=score)
    return subs[0]


def _fetch_title(url: str, cookies: Path | None) -> str:
    cmd = [
        "yt-dlp",
        "--skip-download",
        "--no-playlist",
    ...
    try:
        ...
        print(f"[yt-dlp title] failed: {e}", flush=True)
        return "Untitled video"


def _list_subs(url: str, cookies: Path | None) -> None:
    cmd = [
        "yt-dlp",
        "--list-subs",
        "--no-playlist",
    ...
    try:
        ...
        print(f"[yt-dlp --list-subs rc={proc.returncode}]", flush=True)
        if proc.stdout:
            print(f"[yt-dlp --list-subs stdout]\n{proc.stdout[:4000]}", flush=True)
        if proc.stderr:
            print(f"[yt-dlp --list-subs stderr]\n{proc.stderr[:2000]}", flush=True)
    except Exception as e:
        print(f"[yt-dlp --list-subs] failed: {e}", flush=True)
...
def _run_ytdlp_once(url: str, workdir: Path, cookies: Path | None) -> tuple[Path | None, str, str, int]:
    ...
    sub = _pick_subtitle_file(workdir)
    title = ""
    if sub:
        # filename pattern: "<title> [<id>].<lang>.<ext>"
        m = re.match(r"^(?P<title>.+) \[[a-zA-Z0-9_-]{11}\]\.", sub.name)
        if m:
            title = m.group("title").strip()
    if not title:
        title = _fetch_title(url, cookies)
    if not sub:
        _list_subs(url, cookies)
    return sub, title, proc.stderr or "", proc.returncode
...
def parse_vtt(vtt_text: str) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for m in VTT_BLOCK_RE.finditer(vtt_text):
        lines = [
            re.sub(r"<[^>]+>", "", ln).strip()
...
def parse_json3(text: str) -> list[dict[str, Any]]:
    ...
    for ev in data.get("events", []) or []:
        ...
        out.append({"text": line, "start": start_ms / 1000.0, "duration": dur_ms / 1000.0})
    return out


def load_subtitle(path: Path) -> list[dict[str, Any]]:
    ...
    # try srt then vtt as a fallback
    return parse_srt(text) or parse_vtt(text)
    ...
    try:
        if manual and manual.strip():
            ...
            with tempfile.TemporaryDirectory() as td:
                workdir = Path(td)
                sub_path, title = run_ytdlp(url, workdir)
                if not sub_path:
                    raise RuntimeError(
                        "yt-dlp reached the video but wrote no subtitle files. "
                        "The command may have run in simulation mode or the requested subtitle "
                        "languages were unavailable. Check --list-subs output in the worker logs. "
                        "If YouTube shows a transcript, paste it manually below and HookFinder will analyze it."
                    )
                set_progress(f"Parsing {sub_path.suffix.lstrip('.').upper()} into timestamped segments…")
                segments = load_subtitle(sub_path)
